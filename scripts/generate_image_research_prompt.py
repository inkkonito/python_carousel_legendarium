...
3. Search specifically for the artist named in the slide's "credit" whenever a credit is provided.
4. Search specifically for Tolkien artwork with a dark fantasy, epic, atmospheric, painterly/oil aesthetic.
...
When several valid artworks exist, prefer the one with the strongest combination of:

darkness + epic scale + painterly quality + cinematic atmosphere.
...
Prefer/Ideally (but not only) established Tolkien artists when appropriate, especially:
...
The artwork must actually depict the subject of the slide.
...
def build_prompt(content):
    ...
    return (
        ...
        + "The selected artwork MUST satisfy the strict visual direction: "
          "DARK + EPIC + ATMOSPHERIC + PAINTERLY/OIL + CINEMATIC.\n\n"
        + "Do not use maps, diagrams, infographics, screenshots, generic "
          "fantasy, or visually weak artwork when a suitable Tolkien "
